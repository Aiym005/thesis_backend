...
async function handleRegister(event) {
    ...
    try {
        ...
        const registeredUsername = payload.username ?? username;
        ...
        if (!loginResponse.ok) {
            throw new Error(loginPayload.message || "Бүртгэл үүслээ, гэхдээ автоматаар нэвтэрч чадсангүй.");
        }

        sessionStorage.setItem("thesis.authUser", JSON.stringify(loginPayload.user));
        applyAuthenticatedUser(loginPayload.user);
        await loadDashboard();
        showMessage(loginPayload.message || payload.message || "Амжилттай нэвтэрлээ.", false);
    } catch (error) {
